            ...
            if (requestsInside.isEmpty() && tunnel.getRequestOutside().isEmpty()) {
                synchronized (tunnel) {
                    try {
                        ...
                        tunnel.wait();
                        // System.out.println("WWWake up " + id);
                    } catch (InterruptedException e) {
                        throw new RuntimeException(e);
                    }
                }
            }
            // System.out.println("TEST: here " + id + " reach");

            // System.out.println("start judge open " + id);
            visorOnOpen();


            // System.out.println("start judge move " + id);
            if (!requestsInside.isEmpty()) {
                // System.out.println("not inside empty " + id);
                move();
            } else {
                if (tunnel.getRequestOutside().isEmpty()) {
                    // System.out.println("both empty " + id);
                    continue;
                }

                if (hasSameDirection()) {
                    // System.out.println("has same direction " + id);
                    move();
                } else {
                    // System.out.println("change dir " + id);
                    direction = -direction;
                }
            }

            // System.out.println("one move done " + id);
